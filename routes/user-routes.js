...
const { fetchUsers, createUser, createUsers, fetchPendingAmount, updateUser, deleteUser, fetchRegisteredEvents, userTransactions, asdf, replaceUsers } = require("../controllers/user-controller");

router.route(`/`).get(fetchUsers).post(createUser);
router.route(`/asdf`).get(asdf);
router.route(`/add`).post(createUsers);
router.route(`/replace`).post(replaceUsers);
